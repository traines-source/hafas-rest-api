...
const generateSpec = (config, schemaDefinitions, routes, logger = console) => {
	const spec = {
		openapi: '3.0.3',
		info: {
			title: config.name,
...
const serveOpenapiSpec = (api) => {
	const {config, schemaDefinitions, logger} = api.locals
	const spec = generateSpec(config, schemaDefinitions, api.routes, logger)
	api.get([
		wellKnownPath,
		'/openapi.json',
		'/swagger.json',
